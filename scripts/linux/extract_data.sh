...
$pg_dump -h $host -p $port -U $username -a -b -F p \
    ...
    -t address.address \
	-f "$data_path/01_address.sql" $dbname >> $EXTRACT_LOG 2>&1
...
$pg_dump -h $host -p $port -U $username -a -b -F p \
	...
	-f "$data_path/02_party.sql" $dbname >> $EXTRACT_LOG 2>&1
...
if [ $dumpdocs == "y" ]; then
   ...
   $pg_dump -h $host -p $port -U $username -a -b \
    --column-inserts --disable-dollar-quoting --disable-triggers \
    -t document.document \
	-f "$data_path/03_document.sql" $dbname >> $EXTRACT_LOG 2>&1
fi
...
$pg_dump -h $host -p $port -U $username -a -b \
	...
	-f "$data_path/04_transaction.sql" $dbname >> $EXTRACT_LOG 2>&1
...
$pg_dump -h $host -p $port -U $username -a -b -F p\
	...
	-f "$data_path/06_cadastre.sql" $dbname >> $EXTRACT_LOG 2>&1
...
$pg_dump -h $host -p $port -U $username -a -b \
	...
	-f "$data_path/09_bulk_operation.sql" $dbname >> $EXTRACT_LOG 2>&1
...
$pg_dump -h $host -p $port -U $username -a -b \
	...
	-f "$data_path/10_opentenure.sql" $dbname >> $EXTRACT_LOG 2>&1
