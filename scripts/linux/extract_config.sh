...
$pg_dump -h $host -p $port -U $username -a -b -F p \
	...
	-f "$config_path/reference_tables.sql" $dbname >> $EXTRACT_LOG 2>&1
...
$pg_dump -h $host -p $port -U $username -a -b -F p \
	...
	-f "$config_path/system.sql" $dbname >> $EXTRACT_LOG 2>&1
...
$pg_dump -h $host -p $port -U $username -a -b -F p \
	...
	-f "$config_path/users.sql" $dbname >> $EXTRACT_LOG 2>&1
...
$pg_dump -h $host -p $port -U $username -a -b -F p \
	...
	-f "$config_path/unlocalized_reference_tables.sql" $dbname >> $EXTRACT_LOG 2>&1	
...
$pg_dump -h $host -p $port -U $username -a -b -F p \
	...
	-f "$config_path/business_rules.sql" $dbname >> $EXTRACT_LOG 2>&1
